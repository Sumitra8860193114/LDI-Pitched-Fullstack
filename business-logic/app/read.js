...
read.logic = async (req) => {
    ...
    const [count, requests] = await Promise.all([
        database.client.collection(collections.PITCHES).countDocuments(key),
        database.client.collection(collections.PITCHES).find(key).skip(offset).limit(limit).toArray()
    ]);
